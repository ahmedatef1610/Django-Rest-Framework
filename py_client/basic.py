import requests

# endpoint = "https://httpbin.org/status/200/"
# endpoint = "https://httpbin.org/anything"
# endpoint = "http://localhost:8000/"
# endpoint = "http://127.0.0.1:8000/api/?abc=123"
endpoint = "http://localhost:8000/api/"  # http://127.0.0.1:8000/

# get_response = requests.get(endpoint)  # HTTP Request
# get_response = requests.post(endpoint, data={"title": "Abc123", "content": "Hello world", "price": "abc134"})
# get_response = requests.get(endpoint, params={"abc": 123})
# get_response = requests.post(endpoint, params={"abc": 123}, json={"query": "Hello world"})
# get_response = requests.get(endpoint, params={"abc": 123}, json={"query": "Hello world"})
# get_response = requests.post(endpoint, params={"abc": 123}, json={"query": "Hello world"})
# get_response = requests.post(endpoint, json={"title": "Abc123", "content": "Hello world", "price": "15.00"})
# get_response = requests.post(endpoint, json={"content": "Hello world"})
get_response = requests.post(endpoint, json={"title": "Abc123", "content": "Hello world", "price": "15.00"})

# print(get_response.headers)
print(get_response.text)  # print raw text response
print(get_response.status_code)

# HTTP Request -> HTML
# REST API HTTP Request -> JSON
# JavaScript Object Notation ~ Python Dict
print(get_response.json())
# print(get_response.json()['message'])
# print(get_response.status_code)
